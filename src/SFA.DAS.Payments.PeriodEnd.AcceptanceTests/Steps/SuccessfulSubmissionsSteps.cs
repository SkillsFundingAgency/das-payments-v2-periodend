    ...
    public class SuccessfulSubmissionsSteps : StepsBase
    {
        ...
        public async Task GivenTheProviderMakesASuccessfulSubmission(string submissionStatus)
        {
            ...
            if (submissionStatus.Equals("failed", StringComparison.OrdinalIgnoreCase))
                await MessageSession.Publish<SubmissionJobFailed>(m => { m.Ukprn = TestSession.Ukprn; });
            else
            {
                Container.Resolve<TestPaymentsDataContext>()
                    .CreateSuccessfulSubmissionJob(TestSession.Ukprn, AcademicYear, CollectionPeriod);
                await MessageSession.Publish<SubmissionJobSucceeded>(m => { m.Ukprn = TestSession.Ukprn; });
            }
        }
        ...
        {
            await MessageSession.Publish<ApprenticeshipUpdated>(m => { m.Ukprn = TestSession.Ukprn; });
        }
